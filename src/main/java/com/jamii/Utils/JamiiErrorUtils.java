package com.jamii.Utils;

/**
 * Error utility class containing error codes for the Jamii application.
 * 
 * <p>This class defines all error response codes used throughout the application.
 * Error codes are combined with the response type to form complete error responses.</p>
 */
public class JamiiErrorUtils {

    /**
     * The response type for error messages.
     */
    public static final String RESPONSE_TYPE = JamiiConstants.RESPONSE_TYPE_ERROR_MESSAGE;

    /**
     * Error code 0001.
     */
    public static final String ERROR_CODE_0001 = "0001";
    
    /**
     * Error code 0002.
     */
    public static final String ERROR_CODE_0002 = "0002";
    
    /**
     * Error code 0003.
     */
    public static final String ERROR_CODE_0003 = "0003";
    
    /**
     * Error code 0004.
     */
    public static final String ERROR_CODE_0004 = "0004";
    
    /**
     * Error code 0005.
     */
    public static final String ERROR_CODE_0005 = "0005";
    
    /**
     * Error code 0006.
     */
    public static final String ERROR_CODE_0006 = "0005";
    
    /**
     * Error code 0007.
     */
    public static final String ERROR_CODE_0007 = "0007";
    
    /**
     * Error code 0008.
     */
    public static final String ERROR_CODE_0008 = "0008";
    
    /**
     * Error code 0009.
     */
    public static final String ERROR_CODE_0009 = "0009";
    
    /**
     * Error code 0010.
     */
    public static final String ERROR_CODE_0010 = "0010";
    
    /**
     * Error code 0011.
     */
    public static final String ERROR_CODE_0011 = "0011";
    
    /**
     * Error code 0012.
     */
    public static final String ERROR_CODE_0012 = "0012";
    
    /**
     * Error code 0013.
     */
    public static final String ERROR_CODE_0013 = "0013";
    
    /**
     * Error code 0014.
     */
    public static final String ERROR_CODE_0014 = "0014";
    
    /**
     * Error code 0015.
     */
    public static final String ERROR_CODE_0015 = "0015";
    
    /**
     * Error code 0016.
     */
    public static final String ERROR_CODE_0016 = "0016";
    
    /**
     * Error code 0017.
     */
    public static final String ERROR_CODE_0017 = "0017";
    
    /**
     * Error code 0018.
     */
    public static final String ERROR_CODE_0018 = "0018";
    
    /**
     * Error code 0019.
     */
    public static final String ERROR_CODE_0019 = "0019";
    
    /**
     * Error code 0020.
     */
    public static final String ERROR_CODE_0020 = "0020";
    
    /**
     * Error code 0021.
     */
    public static final String ERROR_CODE_0021 = "0021";
    
    /**
     * Error code 0022.
     */
    public static final String ERROR_CODE_0022 = "0022";
    
    /**
     * Error code 0023.
     */
    public static final String ERROR_CODE_0023 = "0023";
    
    /**
     * Error code 0024.
     */
    public static final String ERROR_CODE_0024 = "0024";
    
    /**
     * Error code 0025.
     */
    public static final String ERROR_CODE_0025 = "0025";
    
    /**
     * Error code 0026.
     */
    public static final String ERROR_CODE_0026 = "0026";
    
    /**
     * Error code 0027.
     */
    public static final String ERROR_CODE_0027 = "0027";
    
    /**
     * Error code 0028.
     */
    public static final String ERROR_CODE_0028 = "0028";
    
    /**
     * Error code 0029.
     */
    public static final String ERROR_CODE_0029 = "0029";
    
    /**
     * Error code 0030.
     */
    public static final String ERROR_CODE_0030 = "0030";
    
    /**
     * Error code 0031.
     */
    public static final String ERROR_CODE_0031 = "0031";
    
    /**
     * Error code 0032.
     */
    public static final String ERROR_CODE_0032 = "0032";
    
    /**
     * Error code 0033.
     */
    public static final String ERROR_CODE_0033 = "0033";
    
    /**
     * Error code 0034.
     */
    public static final String ERROR_CODE_0034 = "0034";
    
    /**
     * Error code 0035.
     */
    public static final String ERROR_CODE_0035 = "0035";
    
    /**
     * Error code 0036.
     */
    public static final String ERROR_CODE_0036 = "0036";
    
    /**
     * Error code 0037.
     */
    public static final String ERROR_CODE_0037 = "0037";
    
    /**
     * Error code 0038.
     */
    public static final String ERROR_CODE_0038 = "0038";
    
    /**
     * Error code 0039.
     */
    public static final String ERROR_CODE_0039 = "0039";
    
    /**
     * Error code 0040.
     */
    public static final String ERROR_CODE_0040 = "0040";
    
    /**
     * Error code 0041.
     */
    public static final String ERROR_CODE_0041 = "0041";
    
    /**
     * Error code 0042.
     */
    public static final String ERROR_CODE_0042 = "0042";
    
    /**
     * Error code 0043.
     */
    public static final String ERROR_CODE_0043 = "0043";
    
    /**
     * Error code 0044.
     */
    public static final String ERROR_CODE_0044 = "0044";
    
    /**
     * Error code 0045.
     */
    public static final String ERROR_CODE_0045 = "0045";
    
    /**
     * Error code 0046.
     */
    public static final String ERROR_CODE_0046 = "0046";
    
    /**
     * Error code 0047.
     */
    public static final String ERROR_CODE_0047 = "0047";
    
    /**
     * Error code 0048.
     */
    public static final String ERROR_CODE_0048 = "0048";
    
    /**
     * Error code 0049.
     */
    public static final String ERROR_CODE_0049 = "0049";
    
    /**
     * Error code 0050.
     */
    public static final String ERROR_CODE_0050 = "0050";


}
